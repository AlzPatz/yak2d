...
float4 main(FragmentIn fIn) : SV_Target
{
	...
	{
		//Colour Only
		return fIn.FColor.r - fIn.FColor.g;
	}

	if (fIn.FTexturingType == 1)
	{
		//Single Texture
		float4 sampleSingle = fIn.FColor * tex0.Sample(texSampler0, fIn.FTexCoord0);
		return sampleSingle.r - sampleSingle.g;        
	}

	//Dual Texture
	float4 sampleDual = fIn.FColor *
	(
		(tex0.Sample(texSampler0, fIn.FTexCoord0) * fIn.FTex0Weight) +
		(tex1.Sample(texSampler1, fIn.FTexCoord1) * (1.0 - fIn.FTex0Weight))
	);
	return sampleDual.r - sampleDual.g;
}