...
// Gold oldMovieFactors.Noise �2015 dcerisano@standard3d.com 
...
float gold_noise(float2 coordinate, float seed) {
	return fract(tan(distance(coordinate*(seed + PHI), float2(PHI, PI)))*SQ2);
}
...
float4 ProcessStatic(float4 colour, float2 tex, constant StaticFactors &staticFactors, constant PixellateFactors &pixellateFactors)
{
	...
	float rnd = rand(staticFactors.Time * pixInt);
	//float rnd = gold_noise(pixInt, staticFactors.Time); //Not sure if allowed to use Gold oldMovieFactors.Noise as has silly (c) symbol...
	//float rnd = rand(texture(Sampler_Noise, staticFactors.Time * pixInt).xy); // Don't really need the extra random sample...
